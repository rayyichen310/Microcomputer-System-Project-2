...
## 🚀 Key Features
* **Dual-timer architecture** (Timer 0 for pitch, Timer 1 for tempo).
* **Accurate tempo control** (♩ = 70 BPM).
* **Real-time square wave generation** via GPIO pin (`P3.4`).
* **Support for rest notes** and variable note durations.
* **Lookup-table based design** to optimize runtime performance.
...
* **Challenge 1: Frequency Calculation Complexity**
  ...
  * *Solution:* Developed an Excel-based automated lookup table (`8051_Project_design.xlsx`) to generate precise `TH0`/`TL0` hex values instantly.
...
## 🚀 Future Extensions
1. **Microcontroller Piano:** Integrate a matrix keypad to allow real-time note input, transforming the board into a simple electronic keyboard.
2. **Multi-Song Player:** Store multiple melodies in arrays and use external interrupts (buttons) to select and switch between tracks dynamically.
